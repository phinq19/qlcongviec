...
namespace WorkLibrary
{
    public class ServerProvider
    {
        ...
        public static DataTable ExecuteStore(string sqlStore,ArrayList parameter, ArrayList value)
        {
            if (mySqlConnection == null)
                Connect();
            ...
            {
                myOleDbCommand.Parameters.Add(new SqlParameter(parameter[i].ToString(),value[i]));
                i++;
            }
            DataSet dtset = new DataSet();
            SqlDataAdapter myOleDataAdapter = new SqlDataAdapter(myOleDbCommand);
            myOleDataAdapter.Fill(dtset);
            return dtset.Tables[0];
